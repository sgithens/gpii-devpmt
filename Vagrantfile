...
# This Vagrantfile and setup is based somewhat upon the gpii-universal vagrant setup, as we need to
# run copies of universal servers for testing, and then expose additional ports for the devpmt
...
host_couchdb_port = ENV["VM_COUCHDB_PORT"] || vars["nodejs_couchdb_port"] || 9082
guest_couchdb_port = vars["nodejs_couchdb_port"] || 9082

# Prepare the port forwarding for the devpmt
host_devpmt_port = ENV["VM_DEVPMT_PORT"] || vars["nodejs_devpmt_port"] || 8085
guest_devpmt_port = vars["nodejs_devpmt_port"] || 8085
...
Vagrant.configure(2) do |config|
  ...
  config.vm.network "forwarded_port", guest: guest_couchdb_port, host: host_couchdb_port, protocol: "tcp",
    auto_correct: true
  config.vm.network "forwarded_port", guest: guest_devpmt_port, host: host_devpmt_port, protocol: "tcp",
    auto_correct: true
